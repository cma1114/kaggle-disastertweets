...
# Split your DataFrame into training and validation sets (75/25)
#train_df, val_df = train_test_split(df, test_size=0.25, random_state=42)
ds = Dataset.from_pandas(df)
ds = ds.train_test_split(test_size=0.05, seed=42)
train_df = ds['train'].to_pandas()
val_df = ds['test'].to_pandas()
train_df[['prompt', 'completion']].to_csv("/Users/christopherackerman/input/train_openai_cln.csv", index=False)
val_df[['prompt', 'completion']].to_csv("/Users/christopherackerman/input/val_openai_cln.csv", index=False)

test_df = pd.read_csv("/Users/christopherackerman/input/test.csv")
...
#openai api completions.create -m curie:ft-personal-2023-04-30-16-24-17 -p <YOUR_PROMPT>
#openai api completions.create -m curie:ft-personal-2023-05-28-22-55-26 -p <YOUR_PROMPT>
...
###do eval set
df = pd.read_csv("/Users/christopherackerman/input/val_openai_cln.csv")
...
first_df = pd.read_csv("/Users/christopherackerman/input/train_cln.csv")
...
### now do test set
df = pd.read_csv("/Users/christopherackerman/input/test_openai_cln.csv")
...
first_df = pd.read_csv("/Users/christopherackerman/input/test.csv")
first_df['keyword'] = first_df['keyword'].fillna('unknown')
first_df['location'] = first_df['location'].fillna('unknown')
sep = " [SEP] "
first_df['text'] = first_df.apply(lambda row: f"{row['text']} {sep} keyword: {row['keyword']} {sep} location: {row['location']}", axis=1)
